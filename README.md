...
- Codex desktop uses the included Node adapter and local desktop IPC. On macOS, its durable
  launchd owner launches or reattaches a persisted task after Desktop restarts. Current Desktop
  builds bring Codex to the foreground while handling that reattach deep link.
...
- Optional OpenSSH signing is verified before PTY/IPC delivery. Signatures remain in the transport
  files; agent context receives only a compact verdict and the canonical message.
...
Routing and mailboxes remain separate concerns. The bundled signer authenticates a complete ring
but does not choose its destination or add a messaging framework. The skill reuses a host's
existing wake adapter when one is already wired.

Authenticated delivery still needs an explicit trust decision: enroll each sender's public key in
an OpenSSH allowed-signers file, then configure the adapter with that file. Doorbell can prove an
enrolled signer; it cannot decide whom you should trust.
