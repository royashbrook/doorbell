...
Optional short names keep one canonical instruction body. After installing `doorbell`, run:

```bash
node doorbell/scripts/install-aliases.mjs --skills-dir /absolute/path/to/your/skills
```

Claude Code exposes `/doorbell`, `/db`, and `/ring`. Codex uses `$doorbell`, `$db`, and `$ring`.
